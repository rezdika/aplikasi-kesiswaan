...
<div class="container-fluid px-4">
    ...
    <div class="card mb-4">
        <div class="card-body">
            <form action="{{ route('kesiswaan.pelanggaran.update', $pelanggaran->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="mb-3">
                    <label>Siswa</label>
                    <select name="siswa_id" class="form-control" required>
                        @foreach($siswas as $s)
                            <option value="{{ $s->id }}" {{ $pelanggaran->siswa_id == $s->id ? 'selected' : '' }}>{{ $s->nama_siswa }} : {{ $s->kelas->nama_kelas }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="mb-3">
                    ...
                    <select name="jenis_pelanggaran_id" id="jenis_pelanggaran_id" class="form-control" required>
                        @foreach($jenisPelanggarans as $jp)
                            <option value="{{ $jp->id }}" data-poin="{{ $jp->poin }}" {{ $pelanggaran->jenis_pelanggaran_id == $jp->id ? 'selected' : '' }}>{{ $jp->nama_pelanggaran }} ({{ $jp->poin }} poin)</option>
                        @endforeach
                    </select>
                </div>
                @if($tahunAjaranAktif)
                ...
                <div class="mb-3">
                    <label>Poin</label>
                    <input type="number" id="poin" class="form-control" value="{{ $pelanggaran->poin }}" readonly style="background-color: #e9ecef;">
                </div>
                <div class="mb-3">
                    <label>Keterangan</label>
                    <textarea name="keterangan" class="form-control">{{ $pelanggaran->keterangan }}</textarea>
                </div>
                <button type="submit" class="btn btn-primary">Update</button>
                <a href="{{ route('kesiswaan.pelanggaran.index') }}" class="btn btn-secondary">Kembali</a>
            </form>
        </div>
    </div>
</div>
