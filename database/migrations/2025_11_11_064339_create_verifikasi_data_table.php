...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('verifikasi_data', function (Blueprint $table) {
            $table->id();
            $table->foreignId('pelanggaran_id')->nullable()->constrained('pelanggaran')->onDelete('cascade');
            $table->foreignId('siswa_id')->nullable()->constrained('siswa')->onDelete('cascade');
            $table->foreignId('guru_id')->constrained('users');
            $table->enum('status', ['menunggu', 'diverifikasi', 'ditolak', 'direvisi'])->default('menunggu');
            $table->timestamps();
        });
    }
